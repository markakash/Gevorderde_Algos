 ...
 * @class Graaf
 *
 * @tparam RT: Duidt aan of de graaf gericht is of niet. Dit beinvloedt functies ivm
 *             verbindingen
 *
 * @brief Graaf is de basisklasse voor een hiërarchie van ijlegraafklassen.
 * Deze hiërarchie zal klassen bevatten voor grafen met data behorend bij knopen en/of
 * takken. Deze data worden opgeslagen in vectoren, zodat kan worden opgezocht op index.
 *
 * *Knopen:* Deze zijn opeenvolgend genummerd. Knopen kunnen niet individueel verwijderd
 * worden. Wel kunnen alle knopen samen verwijderd worden (door wis()), en kunnen knopen
 * één voor één toegevoegd worden.
 *
 * *Verbindingen:* Deze kunnen naar hartenlust toegevoegd en verwijderd worden. Als een
 * verbinding wordt verwijderd, wordt het nummer op de stack van
 * `vrijgekomenVerbindingsnummers` gezet. Worden verbindingen toegevoegd, dan worden eerst
 * deze vrijgekomen nummers opnieuw gebruikt, alvorens nieuwe nummers worden toegevoegd.
 * `hoogsteVerbindingsnummer` kan nooit verkleinen, en de vector met `takdata` zal altijd
 * `hoogsteVerbindingsnummer` elementen bevatten.
 *
...
public:
     ...
     * @return int Geeft verbindingsnummer van toegevoegde verbinding terug (begint bij 0).
     */
    virtual int voegVerbindingToe(int van, int naar);

    /**
     * @brief Verwijdert de verbinding tussen knoopnummers 'van' en 'naar', incl. de terugverbinding indien ongericht.
     *
    ...
    /**
     * @return int Geeft het verbindingsnummer van de verbinding tussen 'van' en 'naar'.
     *
     * Gooit een GraafExceptie indien knoopnummers ongeldig zijn
     *
     * Geeft -1 indien die verbinding niet bestaat.
     ...
     *
     * @param i knoopnummer
     * @return const Burenlijst& Afbeelding van knoopnummer buur of verbindingsnummer
     */
    const Burenlijst &operator[](int i) const { return burenlijsten[i]; }
    /**
     * @brief Niet-const toegang tot burenlijst van knoop. Zo kan deze ook als lvalue gebruikt worden.
     *
     * @param i knoopnummer
    ...
    /// Genereer een .dot bestand dat met Graphviz kan worden omgezet in een afbeelding.
...
void Graaf<RT>::verwijderVerbinding(int van, int naar)
{
    controleerKnoopnummer(van);
    controleerKnoopnummer(naar);
    if (burenlijsten[van].find(naar) != burenlijsten[van].end())
    { //verbinding bestaat
...
public:
    ...
    /**
     * @brief Noot: toevoegfunctie zonder takdata op te geven kan alleen gebruikt als de
     * klasse Takdata een defaultconstructor heeft.
     */
    virtual int voegVerbindingToe(int van, int naar);
